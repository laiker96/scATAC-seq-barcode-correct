...
# print "Mapping reads..."
# mapper = "bowtie2 -p 8 -X 2000 -3 1 -x " + args.genome + " -1" + args.outdir + args.prefix + ".split.1.trimmed.paired.fastq.gz " + " -2" + args.outdir + args.prefix + ".split.2.trimmed.paired.fastq.gz" +" 2> $1.split.bowtie2.log -S h.split.sam"
#+ "; samtools view -bS h.split.sam > h.split.bam; samtools view -h -f3 -F12 -q10 h.split.bam | grep -v '[0-9]'$'\t'chrM | grep -v '[0-9]'$'\t'chrU | grep -v _CTF_ | grep -v _AMBIG_ | samtools view -Su - | samtools sort -@ 8 - h.split.q10.sort; samtools index h.split.q10.sort.bam"
# submitter(mapper)
